...
// FIX DATABASE ALTERVISTA: Controlla e crea colonna is_premium
...
if (!$possiede) die("<div style='background:#020617; color:white; height:100vh; display:flex; flex-direction:column; justify-content:center; align-items:center;'><h1 style='font-size:2rem; margin-bottom:20px;'>Accesso Negato 🔒</h1><a href='dettaglio_libro.php?id=$id_libro' style='background:#06b6d4; color:white; padding:15px 30px; text-decoration:none; border-radius:10px; font-weight:bold;'>Vai al Dettaglio Libro</a></div>");
...
<body class="theme-dark h-screen flex flex-col relative selection:bg-cyan-500 selection:text-white" id="readerBody">
    ...
    <?php if ($esiste_pdf): ?>
        ...
        <div id="paywall-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center dict-hidden">
            <div class="bg-gradient-to-br from-slate-900 to-[#020617] p-8 rounded-[2rem] w-[90%] max-w-md border border-amber-500/50 text-center shadow-[0_0_50px_rgba(245,158,11,0.2)] relative overflow-hidden">
                ...
                <ul class="text-left text-xs text-slate-400 space-y-2 mb-8">
                    ...
                    <li><span class="text-emerald-400 mr-2">✓</span> Sincronizzazione Cloud su tutti i dispositivi</li>
                    <li><span class="text-emerald-400 mr-2">✓</span> Statistiche WPM e Lounge Zen</li>
                </ul>
                <a href="checkout_premium.php?return_id=<?= $id_libro ?>" class="block w-full bg-gradient-to-r from-amber-500 to-yellow-600 text-white font-black py-3 rounded-xl uppercase tracking-widest shadow-lg hover:scale-105 transition-transform">Diventa Pro a 4,99€</a>
        ...
        <div id="tts-player" class="absolute top-16 md:top-20 left-1/2 -translate-x-1/2 w-[95%] max-w-4xl glass-panel rounded-2xl md:rounded-3xl p-4 md:p-6 shadow-2xl z-40 transition-all duration-500 transform -translate-y-[150%] opacity-0 border border-cyan-500/30 flex flex-col max-h-[80vh]">
            <div class="flex flex-wrap justify-between items-center gap-3 mb-4 shrink-0">
                <div class="flex items-center gap-2 md:gap-3">
                    ...
                    <div><h3 class="text-white font-black uppercase tracking-widest text-[10px] md:text-sm flex items-center gap-2">Lettore AI <span class="w-2 h-2 bg-emerald-500 rounded-full animate-pulse inline-block"></span></h3><p class="text-[10px] md:text-xs text-slate-400">Doppio click x Dizionario | Seleziona x Evidenziare</p></div>
                </div>
            ...
            <div id="subtitles-container" class="bg-black/60 rounded-xl md:rounded-2xl p-4 border border-white/5 text-slate-400 text-sm md:text-[1.1rem] font-medium leading-relaxed relative flex-1" onmouseup="showHighlightMenu(event)"><p id="tts-text" class="transition-all duration-200">Pronto per estrarre il testo...</p></div>
        ...
        <main id="pdf-container">
            <div id="canvas-wrapper">
                <div id="dog-ear" onclick="toggleBookmark()" title="Aggiungi/Rimuovi Segnalibro"></div>
    ...
    <?php else: ?>
        <main class="flex-1 flex flex-col items-center justify-center p-6 text-center bg-[#020617]"><div class="w-20 h-20 md:w-24 md:h-24 mb-6 rounded-full bg-slate-800 border border-slate-600 flex items-center justify-center text-3xl md:text-4xl shadow-lg">📁</div><h2 class="text-xl md:text-2xl font-black text-white mb-2 tracking-tight">PDF Non Trovato</h2><p class="text-sm md:text-base text-slate-400 max-w-md mb-6">Il file digitale di questo libro non è ancora stato caricato nel server.</p></main>
    <?php endif; ?>
